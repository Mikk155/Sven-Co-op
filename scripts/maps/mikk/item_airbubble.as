...
class item_airbubble : ScriptBaseEntity 
{
	void Precache() 
	{
		...
		{
			g_Game.PrecacheGeneric( "sound/" + string(self.pev.message) );
			g_SoundSystem.PrecacheSound( string(self.pev.message) );
		}
	}
	...
	void Touch( CBaseEntity@ pOther ) 
	{
		...
		g_SoundSystem.EmitSoundDyn( pOther.edict(), CHAN_ITEM, ( string( self.pev.message ).IsEmpty() ) ? "debris/bustflesh1.wav" : string( self.pev.message ), 1.0, ATTN_NORM, 0, PITCH_HIGH );
