...
void RegisterAmmoIndividual()
{
	...
	DefaultRegister( "weapon_snarks_individual" );
	DefaultRegister( "weapon_handgrenade_individual" );
	DefaultRegister( "weapon_tripmine_individual" );
	DefaultRegister( "item_battery_individual" );
	DefaultRegister( "item_healthkit_individual" );
	DefaultRegister( "weaponbox_individual" );
	DefaultRegister( "weapon_satchel_individual" );
//	DefaultRegister( "func_recharge_individual" );
//	DefaultRegister( "func_healthcharger_individual" );
}
...
void AmmoIndividualRemap()
{
	...
	Reemplazar( "weapon_snark", "weapon_snarks_individual" );
	Reemplazar( "weapon_handgrenade", "weapon_handgrenade_individual" );
	Reemplazar( "weapon_tripmine", "weapon_tripmine_individual" );
	Reemplazar( "item_battery", "item_battery_individual" );
	Reemplazar( "item_healthkit", "item_healthkit_individual" );
	Reemplazar( "weapon_satchel", "weapon_satchel_individual" );
}
/*
class CBaseKeepData
{
	float juice = g_EngineFuncs.CVarGetPointer( "sk_suitcharger" ).value;
	float flNextCharge = 0;

    float juice2 = g_EngineFuncs.CVarGetPointer( "sk_healthcharger" ).value;
    float flNextCharge2 = 0;
}

class func_recharge_individual : ScriptBaseEntity
{
	CBaseKeepData pModify;

	dictionary dicPlayerJuice;

	private string CustomStartSound	 = "items/suitchargeok1.wav";
	private string CustomLoopSound	 = "items/suitcharge1.wav";
	private string CustomDeniedSound = "items/suitchargeno1.wav";
	private string TriggerOnEmpty = "";

	private float CustomJuice = g_EngineFuncs.CVarGetPointer( "sk_suitcharger" ).value;
	private float CustomRechargeTime = 0.2f;

	bool KeyValue( const string& in szKey, const string& in szValue ) 
	{
        if(szKey == "CustomJuice")// Se sobre pone al skl cvar
            pModify.juice = CustomJuice = atof(szValue);
        else if(szKey == "CustomRechargeTime" )// tiempo de recarga supongo idk
            CustomRechargeTime = atof(szValue);
        else if(szKey == "TriggerOnEmpty")// trigger al pActivator terminar de usar todo su jugo
            TriggerOnEmpty = szValue;
        else if(szKey == "CustomStartSound")
            CustomStartSound = szValue;
        else if(szKey == "CustomLoopSound")
            CustomLoopSound = szValue;
        else if(szKey == "CustomDeniedSound")
            CustomDeniedSound = szValue;
        else
	...
	void Precache()
	{
		g_SoundSystem.PrecacheSound( CustomDeniedSound );
		g_SoundSystem.PrecacheSound( CustomLoopSound );
		g_SoundSystem.PrecacheSound( CustomStartSound );
		
		g_Game.PrecacheGeneric( 'sound/' + CustomDeniedSound );
		g_Game.PrecacheGeneric( 'sound/' + CustomLoopSound );
		g_Game.PrecacheGeneric( 'sound/' + CustomStartSound );
	...
	void Use( CBaseEntity@ pActivator, CBaseEntity@ pCaller, USE_TYPE useType, float value )
	{
		...
		CBaseKeepData@ pData = GetPlayerJuice(cast<CBasePlayer@>(pActivator));
		
		if(pData.juice > 0)
		{
			if(pActivator.TakeArmor(1, DMG_GENERIC) && pData.flNextCharge < g_Engine.time)
			{
				--pData.juice;
				pData.flNextCharge = g_Engine.time + CustomRechargeTime;
			}
		}
		else if( TriggerOnEmpty != "" )
		{
			g_EntityFuncs.FireTargets( TriggerOnEmpty, pActivator, pActivator, USE_TOGGLE );
		}
	}
	...
	CBaseKeepData@ GetPlayerJuice(CBasePlayer@ pPlayer)
	{
		...
		if(!dicPlayerJuice.exists(SteamID))
		{
			CBaseKeepData pData;
			dicPlayerJuice[SteamID] = pData;
		}
		return cast<CBaseKeepData@>(dicPlayerJuice[SteamID]);
	}
}

class func_healthcharger_individual : ScriptBaseEntity
{
	CBaseKeepData pModify;
	...
	private float CustomJuice = g_EngineFuncs.CVarGetPointer( "sk_healthcharger" ).value;
	private float CustomRechargeTime = 0.2f;

	bool KeyValue( const string& in szKey, const string& in szValue ) 
	{
        if(szKey == "CustomJuice")// Se sobre pone al skl cvar
            pModify.juice2 = CustomJuice = atof(szValue);
        else if(szKey == "CustomRechargeTime" )// tiempo de recarga supongo idk
            CustomRechargeTime = atof(szValue);
        else if(szKey == "TriggerOnEmpty")// trigger al pActivator terminar de usar todo su jugo
            TriggerOnEmpty = szValue;
	...
	void Use( CBaseEntity@ pActivator, CBaseEntity@ pCaller, USE_TYPE useType, float value )
	{
		...
		if(pData.juice2 > 0)
		{
			if(pActivator.TakeHealth(1, DMG_GENERIC) && pData.flNextCharge2 < g_Engine.time)
			{
				--pData.juice2;
				pData.flNextCharge2 = g_Engine.time + CustomRechargeTime;
			}
		}
		else if( TriggerOnEmpty != "" )
		{
...
{	
	private int m_il9mm, m_ilbuckshot, m_ilARgrenades, m_ilTM, m_ilHG, m_ilsnarks, m_ilsporeclip, m_ilrockets, m_iluranium, m_ilbolts, m_il556, m_il357, m_ilm40a1, m_ilsatchel;
	...
	bool KeyValue( const string& in szKey, const string& in szValue ) 
	{
		...
		else if( szKey == "satchel" )
		{
			m_ilsatchel = atoi( szValue );
			return true;
		}
		else 
			return BaseClass.KeyValue( szKey, szValue );
...
class weapon_snarks_individual : ScriptBasePlayerItemEntity, ammo_base_c
{
	void Spawn()
	{
...
class weapon_satchel_individual : ScriptBasePlayerItemEntity, ammo_base_c
{
	void Spawn()
	{
...
class weapon_handgrenade_individual : ScriptBasePlayerItemEntity, ammo_base_c 
{
	void Spawn()
	{
...
class weapon_tripmine_individual : ScriptBasePlayerItemEntity, ammo_base_c 
{
	void Spawn()
	{
