...
namespace env_fog_custom
{
    void Register()
    {
        g_CustomEntityFuncs.RegisterCustomEntity( 'env_fog_custom::env_fog_custom','env_fog_custom' );
            ...
            if( pFog !is null )
            {
                ...
                or pFog.pev.SpawnFlagBitSet( FADEIN_MAXDIS ) )
                {
                    dictionary g_keyvalues =
                    {
                        ...
                        { "$v_fog_rendercolor", g_Util.GetCKV( pFog, '$v_fog_rendercolor' ) },
                        { "$f_fog_rendercolor_time", g_Util.GetCKV( pFog, '$f_fog_rendercolor_time' ) },
                        { "$i_fog_iuser2", g_Util.GetCKV( pFog, '$i_fog_iuser2' ) },
                        { "$f_fog_iuser2_time", g_Util.GetCKV( pFog, '$f_fog_iuser2_time' ) },
                        { "$i_fog_iuser3", g_Util.GetCKV( pFog, '$i_fog_iuser3' ) },
                        { "$f_fog_iuser3_time", g_Util.GetCKV( pFog, '$f_fog_iuser3_time' ) }
                    };
    ...
    class env_fog_custom : ScriptBaseEntity, ScriptBaseCustomEntity
    {
        ...
        void ExecFade( CBasePlayer@ pPlayer, bool State, USE_TYPE UseType = USE_TOGGLE )
        {
            if( UseType == USE_SET )
            {
                g_Util.SetCKV( pPlayer, '$i_fog_state', 2 );
                return;
            }

            if( atoi( g_Util.GetCKV( pPlayer, '$i_fog_state' ) ) == 1 )
            {
                return;
            }

            g_Util.SetCKV( pPlayer, '$i_fog_state', 1 );
        ...
        void CFade( CBasePlayer@ pPlayer, int Mode, bool Statex )
        {
            ...
            if( !Statex )
            {
                R = uint8( g_Util.StringToVec( g_Util.GetCKV( self, '$v_fog_rendercolor' ) ).x );
                G = uint8( g_Util.StringToVec( g_Util.GetCKV( self, '$v_fog_rendercolor' ) ).y );
                B = uint8( g_Util.StringToVec( g_Util.GetCKV( self, '$v_fog_rendercolor' ) ).z );
                Iuser2 = atoi( g_Util.GetCKV( self, '$i_fog_iuser2' ) );
                Iuser3 = atoi( g_Util.GetCKV( self, '$i_fog_iuser3' ) );
            }

            if( Mode == FADEIN_COLOR )
            {
                Vector VecColor = g_Util.StringToVec( g_Util.GetCKV( pPlayer, '$v_fog_rendercolor' ) );
            ...
            if( !bl ) g_Util.SetCKV( pPlayer, '$i_fog_state', 0 );
        }
        ...
        {
            if( g_Util.GetCKV( self, key ).IsEmpty() )
                return false;
            return true;
        }
    }
