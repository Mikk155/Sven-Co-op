...
namespace trigger_percent
{
    class trigger_percent : ScriptBaseEntity, ScriptBaseCustomEntity
    {
		...
		{
			if( pActivator !is null && pActivator.IsPlayer() && g_Util.GetCKV( pActivator, '$i_triggerpercent_' + self.entindex() ).IsEmpty() )
			{
				++fPlayersTriggered;
	...
	bool Register = g_Util.CustomEntity( 'trigger_percent::trigger_percent','trigger_percent' );
}