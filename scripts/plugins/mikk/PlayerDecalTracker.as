...
void PluginInit()
{
    ...
    mk.FileManager.GetMultiLanguageMessages( msg, 'scripts/plugins/mikk/PlayerDecalTracker.ini' );
}
				...
				if( tr.flFraction < 1.0 )
				{
					...
					if( pNearest !is null )
					{
						...
						if( ( pNearest.Position - tr.vecEndPos ).Length() <= g_flMaxDistance )
						{
							...
							mk.PlayerFuncs.PrintMessage
							(
								pPlayer, dictionary( msg[ 'advice' ] ), CMKPlayerFuncs_PRINT_BIND, false,
								{
									{ '$name$' , string( pNearest.PlayerName ) }
								}
							);
							...
							if( pPlayer.pev.button & IN_USE != 0 )
							{
								mk.PlayerFuncs.PrintMessage
								(
									pPlayer, dictionary( msg[ 'show_author' ] ), CMKPlayerFuncs_PRINT_CHAT, false,
									{
										{ '$name$' , string( pNearest.PlayerName ) },
										{ '$steamid$' , string( pNearest.AuthId ) }
									}
								);
							}
						}
