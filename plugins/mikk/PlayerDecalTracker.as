// Press E to get the information printed onto console -Mikk
/*
...
void PluginInit()
{
	...
	// Removed, so everyone can report porn sprays
			...
			if( tr.flFraction < 1.0 )
			{
				...
				if( pNearest !is null )
				{
					...
					if( ( pNearest.Position - tr.vecEndPos ).Length() <= g_flMaxDistance )
					{
						...
						g_PlayerFuncs.PrintKeyBindingString( pPlayer, "Hold +use to print " +pNearest.PlayerName+"'s ID." );

            			if( pPlayer.pev.button & IN_USE != 0 )
						{
							g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, '\n\n[PlayerDecalTracker] Spray by "' + pNearest.PlayerName + '" Auth ID: "' + pNearest.AuthId + '"\n\n\n' );
						}
					}
				}
			}
