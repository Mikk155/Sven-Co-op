...
#define JSON_CACHE "svencoop_addon/aslp.as.predefined.json"
#define JSON_INDENTATION 4
#define AS_PREDEFINED "svencoop_addon/aslp.as.predefined"
#endif
...
#ifdef AS_GENERATE_DOCUMENTATION
inline File DocWrite( str content = nullptr )
{
    File as_predefined( AS_PREDEFINED );
    as_predefined.Engine = false;
    as_predefined.Base = true;
    if( content != nullptr )
        as_predefined.Append( content );
    return as_predefined;
}
#endif

inline void aslp_SetNamespace( str name, str description, int* namespaces, CASDocumentation* pASDoc )
{
#ifdef AS_GENERATE_DOCUMENTATION
    File file = DocWrite();
    ...
    if( *name != '\0' )
    {
        ...
        size_t p, s = 0;
        while( ( p = ns.find( "::", s ) ) != std::string::npos ) {
            r.push_back( ns.substr( s, p - s ) );
            s = p + 2;
        }
        r.push_back( ns.substr(s) );

        for( const std::string& nameSpace : r )
        {
            (*namespaces)++;
            file.Append( fmt::format( "namespace {}\n{}\n", nameSpace, "{" ) );
        }
    }
#endif

    ASEXT_SetDefaultNamespace( pASDoc, name );
}

inline void aslp_Pragma( str word, str description, CScriptBuilder* pScriptBuilder, bool shouldRegister )
{
// We use a cache file since ASEXT_RegisterScriptBuilderDefineCallback calls the lambda for each AS module
#ifdef AS_GENERATE_DOCUMENTATION
    std::string content;
    File file( JSON_CACHE );
    file.Engine = false;
    file.Base = true;
    file.Read( content );
    auto cache = nlohmann::json::parse( (char*)content.c_str(), nullptr, false, true, true );
    auto obj = cache.value( "preprocessors", nlohmann::json::object() );
    obj[ word ] = description;
    cache[ "preprocessors" ] = obj;
    file.Write( cache.dump( JSON_INDENTATION ) );
#endif

    if( shouldRegister )
    {
        ASEXT_CScriptBuilder_DefineWord( pScriptBuilder, word );
    }
}

inline void aslp_GlobalFunction( str name, str doc, CASDocumentation *pASDoc, void *pfn, int type )
{
#ifdef AS_GENERATE_DOCUMENTATION
    File as_predefined( AS_PREDEFINED );
    as_predefined.Engine = false;
    as_predefined.Base = true;
    as_predefined.Append( doc );
    as_predefined.Append( fmt::format( "{};\n", name ) );
#endif

    ASEXT_RegisterGlobalFunction( pASDoc, doc, name, pfn, type );
}

inline void aslp_Object( str name, str doc, CASDocumentation *pASDoc, int unk, unsigned int flags )
{
#ifdef AS_GENERATE_DOCUMENTATION
    File as_predefined( AS_PREDEFINED );
    as_predefined.Engine = false;
    as_predefined.Base = true;
    as_predefined.Append( doc );
    as_predefined.Append( fmt::format( "class {}\n{}", name, "{" ) );
#endif
    ASEXT_RegisterObjectType( pASDoc, doc, name, unk, flags );
}

void RegisterAngelScriptMethods() 
{
#ifdef AS_GENERATE_DOCUMENTATION
File json_cache( JSON_CACHE );
json_cache.Engine = false;
json_cache.Base = true;
json_cache.Write( "{}" );
...
std::string currentTime;
{
    ...
    currentTime = std::string( time_buffer );
}

// -TODO More information maybe get git commit, tags and more
as_predefined.Write( fmt::format( R"(/**
 ...
 *  Generation date: {}
 *
 *  Refeer to: https://github.com/Mikk155/Sven-Co-op
 */
)", currentTime ) );
#endif
...
{
#pragma region zzzz
bool regDebug = false, regRelease = false, regLinux = false, regWindows = false;
#if _DEBUG
regDebug = true;
#else
regRelease = true;
...
#if _DEBUG
bool regDebug = true;
bool regRelease = false;
#else
bool regDebug = false;
bool regRelease = true;
#endif
#if LINUX
bool regLinux = true;
bool regWindows = false;
#elif WIN32
bool regLinux = false;
bool regWindows = true;
#endif
*/
#pragma endregion
aslp_Pragma( "METAMOD_PLUGIN_ASLP", "//aslp", pScriptBuilder, true );
aslp_Pragma( "METAMOD_DEBUG", "// debug", pScriptBuilder, regDebug );
aslp_Pragma( "METAMOD_RELEASE", "// release", pScriptBuilder, regRelease );
aslp_Pragma( "LINUX", "// linus", pScriptBuilder, regLinux );
aslp_Pragma( "WINDOWS", "// windows", pScriptBuilder, regWindows );
} );
...
aslp_SetNamespace( NAMESPACE_ASLP "::json", R"(/**
 * @brief Json namespace
 * 
 */
)", &nameSpaces, pASDoc );

aslp_GlobalFunction( "bool Deserialize( const string &in str, dictionary &out obj )", R"(/**
 * @brief Deserializes str into obj using json syntaxis
 * str: If ends with .json it will be a file to open.
 */
)", pASDoc, (void*)CASJsonDeserialize, asCALL_CDECL );

aslp_SetNamespace( NAMESPACE_ASLP, R"(/**
 * @brief AngelScript Extended Limitless Potential API
 */
)", &nameSpaces, pASDoc );

#pragma region physent_t
aslp_Object( "PhysicalEntity", R"(/**
 * @brief Physical entity used in the player movement code
 */
)", pASDoc, 0, asOBJ_REF | asOBJ_NOCOUNT );
...
aslp_SetNamespace( "", nullptr, &nameSpaces, pASDoc );
    } );
}
