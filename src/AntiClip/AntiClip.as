...
void Command( const CCommand@ args )
{
    ...
    if( args[1] == "on" || args[1] == "true" || atoi( args[1] ) == 1 )
    {
        if( !hooks )
        {
            Language::PrintAll( pJson[ "ENABLED", {} ], MKLANG::CHAT, { { "name", string( pPlayer.pev.netname ) } }, "[Anti-Clip]" );
            ToggleState( 1 );
        }
    }
    else if( hooks )
    {
        Language::PrintAll( pJson[ "DISABLED", {} ], MKLANG::CHAT, { { "name", string( pPlayer.pev.netname ) } }, "[Anti-Clip]" );
        ToggleState( 0 );
    }
}
