...
namespace ammo_custom
{
    void Register()
    {
        g_Util.CustomEntity( 'ammo_custom' );
    ...
    class ammo_custom : ScriptBasePlayerAmmoEntity, ScriptBaseCustomEntity
    {
        ...
        void Spawn()
        {
            ...
            string renderamt = g_Util.CKV( self, '$i_renderamt' );
            string rendermode = g_Util.CKV( self, '$i_rendermode' );
            string renderfx = g_Util.CKV( self, '$i_renderfx' );
            string rendercolor = g_Util.CKV( self, '$s_rendercolor' );
        ...
        void Use( CBaseEntity@ pActivator, CBaseEntity@ pCaller, USE_TYPE useType, float fldelay )
        {
            if( PlayerAble( pActivator ) )
            {
                if( useType == USE_OFF )
                {
                    g_Util.CKV( pActivator, '$i_' + EntIndex(), 0 );
        ...
        {
            int iValue = atoi( g_Util.CKV( pPlayer, '$i_' + EntIndex() ) );
            g_Util.CKV( pPlayer, '$i_' + EntIndex(), iValue - 1 );
                ...
                for( uint i = 0; i < Weapons.length(); i++ )
                {
                    if( m_iszAmmoName == Weapons[i][0] )
                    {
                        if( cast<CBasePlayer@>( pPlayer ).HasNamedPlayerItem( Weapons[i][1] ) is null )
                        {
                            ...
                            FakeWeapon.pev.spawnflags = DISABLE_RESPAWN;
                ...
                if( m_iszAmmoName == 'flashlight' )
                {
                    if( pPlayer.m_iFlashBattery < 100 )
                    {
                        ...
                        if( flash >= 100 )
                        {
                            ...
                            g_Util.CKV( pPlayer, '$f_pf_flashlight', 100.0 );
                        }
                        else
                        {
                            pPlayer.m_iFlashBattery = int( flash );
                            g_Util.CKV( pPlayer, '$f_pf_flashlight', flash );
                        }
        ...
        bool Pickup( CBasePlayer@ &in pPlayer = null )
        {
            if( !m_iszPickupSound.IsEmpty() )
            {
                ...
                NetworkMessage message( MSG_ONE, NetworkMessages::ItemPickup, pPlayer.edict() );
                    message.WriteString( 'item_' + m_iszAmmoName );
                message.End();
